...
def guard():
    if request.path.startswith('/api/'):
        ...
        now = time.time(); key = request.headers.get('X-Forwarded-For', request.remote_addr or 'unknown')
        with _lock:
            values = [t for t in _hits[key] if now - t < WINDOW]
            if len(values) >= LIMIT:
                return jsonify(error="rate limit exceeded"), 429
            values.append(now); _hits[key] = values
...
@app.get('/api/servers')
def servers():
    return jsonify(servers=[{'id':'cloudflare-global','name':'Cloudflare Global','region':'Global','endpoint':'https://speed.cloudflare.com','active':True}])
